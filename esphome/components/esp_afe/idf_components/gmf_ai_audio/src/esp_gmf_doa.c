...
static esp_gmf_err_t gmf_doa_new(void *cfg, esp_gmf_obj_handle_t *handle)
{
    ...
    *handle = NULL;
    ...
    ESP_LOGI(TAG, "gmf_doa_new - Config - Sample rate: %d, Resolution: %f, d_mics: %f", doa_cfg->sample_rate, doa_cfg->resolution, doa_cfg->d_mics);
...
static inline esp_gmf_job_err_t doa_validate_microphone_format_check(const char *input_format, size_t format_len, uint8_t *m_idx)
{
...
static esp_gmf_job_err_t gmf_doa_open(esp_gmf_audio_element_handle_t self, void *para)
{
    ...
    if ((doa_cfg->frame_ms == 0) || (doa_cfg->sample_rate <= 0) || (format_len == 0)) {
        ESP_LOGE(TAG, "Invalid config: frame_ms=%u, sample_rate=%d, format_len=%zu",
                 doa_cfg->frame_ms, doa_cfg->sample_rate, format_len);
        return ESP_GMF_JOB_ERR_FAIL;
    }
    ...
    gmf_doa->doa_handle = esp_doa_create(doa_cfg->sample_rate, doa_cfg->resolution, doa_cfg->d_mics,
    ...
    ESP_GMF_NULL_CHECK(TAG, gmf_doa->cache, {ret = ESP_GMF_JOB_ERR_FAIL; goto __quit;});

    gmf_doa->mic_left = (int16_t *)esp_gmf_oal_calloc(1, bytes_per_channel);
    ESP_GMF_NULL_CHECK(TAG, gmf_doa->mic_left, {ret = ESP_GMF_JOB_ERR_FAIL; goto __quit;});
    gmf_doa->mic_right = (int16_t *)esp_gmf_oal_calloc(1, bytes_per_channel);
    ESP_GMF_NULL_CHECK(TAG, gmf_doa->mic_right, {ret = ESP_GMF_JOB_ERR_FAIL; goto __quit;});
...
esp_gmf_err_t esp_gmf_doa_init(esp_gmf_doa_cfg_t *config, esp_gmf_obj_handle_t *handle)
{
    ...
    *handle = NULL;
    ...
    ESP_GMF_MEM_VERIFY(TAG, gmf_doa, {return ESP_GMF_ERR_MEMORY_LACK;}, "doa", sizeof(gmf_doa_t));
    ...
    ESP_LOGI(TAG, "esp_gmf_doa_init - Config - Sample rate: %d, Resolution: %f, d_mics: %f", obj_cfg->sample_rate, obj_cfg->resolution, obj_cfg->d_mics);
...
esp_gmf_err_t esp_gmf_doa_set_result_cb(esp_gmf_element_handle_t self,

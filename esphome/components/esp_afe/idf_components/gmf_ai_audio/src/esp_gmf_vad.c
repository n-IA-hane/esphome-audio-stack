...
static esp_gmf_job_err_t gmf_vad_open(esp_gmf_audio_element_handle_t self, void *para)
{
    ...
    ESP_LOGI(TAG, "VADNet config: model_name=%s, vad_mode=%d, min_speech_ms=%d, min_noise_ms=%d",
             model_name, cfg->vad_mode, min_speech_ms, min_noise_ms);
    ...
    ESP_GMF_ELEMENT_OUT_PORT_ATTR_SET(ESP_GMF_ELEMENT_GET(self)->out_attr, ESP_GMF_EL_PORT_CAP_SINGLE, 16, 0,
                                      ESP_GMF_PORT_TYPE_BLOCK | ESP_GMF_PORT_TYPE_BYTE, gmf_vad->frame_bytes);
#if CONFIG_SR_VADN_VADNET1_MEDIUM
    ESP_LOGI(TAG, "GMF VAD open, backend: vadnet, sample_rate: %d, frame_ms: %d, frame_bytes: %lu",
             cfg->sample_rate, cfg->frame_ms, (unsigned long)gmf_vad->frame_bytes);
#else
    ESP_LOGI(TAG, "GMF VAD open, backend: webrtc, sample_rate: %d, frame_ms: %d, frame_bytes: %lu",
             cfg->sample_rate, cfg->frame_ms, (unsigned long)gmf_vad->frame_bytes);
#endif
...
#else
    vad_state_t vad_state = vad_process(gmf_vad->vad_handle, (int16_t *)cache_load->buf, cfg->sample_rate, cfg->frame_ms);
...
esp_gmf_err_t esp_gmf_vad_init(esp_gmf_vad_cfg_t *config, esp_gmf_obj_handle_t *handle)
{
    ...
    ESP_GMF_RET_ON_NOT_OK(TAG, ret, { return ret; }, "Invalid VAD configuration");
    ESP_GMF_NULL_CHECK(TAG, handle, { return ESP_GMF_ERR_INVALID_ARG; });
    *handle = NULL;

    gmf_vad_t *gmf_vad = esp_gmf_oal_calloc(1, sizeof(gmf_vad_t));
    ESP_GMF_MEM_VERIFY(TAG, gmf_vad, { return ESP_GMF_ERR_MEMORY_LACK; }, "vad", sizeof(gmf_vad_t));
...
esp_gmf_err_t esp_gmf_vad_set_result_cb(esp_gmf_element_handle_t self,
                                        esp_gmf_vad_result_callback_t result_callback,
                                        void *ctx)
{
    ESP_GMF_NULL_CHECK(TAG, self, { return ESP_GMF_ERR_INVALID_ARG; });
    esp_gmf_vad_cfg_t *cfg = OBJ_GET_CFG(self);

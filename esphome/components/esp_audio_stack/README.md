...
| `idle_teardown` | bool | true | Delete I2S channels (and their DMA) when the last mic/speaker consumer leaves. Set `false` on no-codec satellites that stop the VA mic and restart microWakeWord in the same second; a second DMA alloc often fails with `ESP_ERR_NO_MEM`. |
